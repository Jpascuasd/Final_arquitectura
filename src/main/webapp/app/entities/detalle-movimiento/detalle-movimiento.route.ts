...
export class DetalleMovimientoResolve implements Resolve<IDetalleMovimiento> {
  constructor(private service: DetalleMovimientoService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDetalleMovimiento> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((detalleMovimiento: HttpResponse<DetalleMovimiento>) => {
...
export const detalleMovimientoRoute: Routes = [
  {
    ...
    data: {
      ...
      pageTitle: 'fleastoreApp.detalleMovimiento.home.title'
    },
