...
describe('Producto e2e test', () => {
  ...
  it('should load Productos', async () => {
    ...
    expect(await productoComponentsPage.getTitle()).to.eq('fleastoreApp.inventariomicroservicioProducto.home.title');
  ...
  it('should load create Producto page', async () => {
    ...
    expect(await productoUpdatePage.getPageTitle()).to.eq('fleastoreApp.inventariomicroservicioProducto.home.createOrEditLabel');
  ...
  it('should create and save Productos', async () => {
    ...
    await promise.all([
      ...
      productoUpdatePage.setReferenciaInput('referencia'),
      productoUpdatePage.setDescripcionInput('descripcion'),
      productoUpdatePage.setCodigoInput('codigo'),
      productoUpdatePage.setProveedorInput('5')
    ]);

    expect(await productoUpdatePage.getNombreInput()).to.eq('nombre', 'Expected Nombre value to be equals to nombre');
    expect(await productoUpdatePage.getReferenciaInput()).to.eq('referencia', 'Expected Referencia value to be equals to referencia');
    ...
    expect(await productoUpdatePage.getCodigoInput()).to.eq('codigo', 'Expected Codigo value to be equals to codigo');
    expect(await productoUpdatePage.getProveedorInput()).to.eq('5', 'Expected proveedor value to be equals to 5');
  ...
  it('should delete last Producto', async () => {
    ...
    expect(await productoDeleteDialog.getDialogTitle()).to.eq('fleastoreApp.inventariomicroservicioProducto.delete.question');
